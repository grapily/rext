...
Rext.prototype.createVersion = function (options, callback) {
  var versionPath = path.join(this.path, options.name, options.version)
    , latestPath = path.join(this.path, options.name, Rext.LATESTDIR)
    , filePath = path.join(this.path, options.name, options.version, Rext.FILENAME)
    , that = this
    ;
  ...
  fs.mkdir(versionPath, function(err) {
    ...
    fs.symlink(options.version, latestPath, function (err) {
      if (err) {
        callback(err);
        return;
      }
