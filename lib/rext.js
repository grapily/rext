...
/**
 * Destroy 'options.version' of 'options.name' document.
...
Rext.prototype.destroy = function (options, callback) {
  var name = options.name
    ...
    , destroyingPath = path.join(docPath, version)
  ...
  async.waterfall([
    ...
    function (next) {
      testPath(destroyingPath, verErrorMes, next);
    },
    function (next) {
      that.latest(name, next);
    },
    function (latestVer, next) {
      if (version && (version === Rext.LATESTDIR || compare(latestVer, version) === 0)) {
        that.updateLatest(name, latestVer, next);
      }
    ...
    rimraf(destroyingPath, callback)
  });
};
