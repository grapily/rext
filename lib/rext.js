 ...
 */
var Rext = module.exports = function (repo) {
  this.repo = path.normalize(repo);
  if (!path.existsSync(this.repo) || !fs.readdirSync(this.repo).length) {
    throw new Error('Path not exists or the directory is not empty');
  };
};
